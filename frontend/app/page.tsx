                    ...
                    <button disabled={welfareLoading}
                      onClick={async () => {
                        ...
                        try {
                          ...
                          alert(`פקודה נוצרה! ${res.reference_num}`);
                          // Auto-generate & download treasurer PDF (non-blocking)
                          const savedB64 = welfareFileB64;
                          const savedMonth = welfareResult.month;
                          const savedYear = welfareResult.year || new Date().getFullYear();
                          const savedMuniName = muni.name;
                          setWelfareResult(null); setWelfareFileB64(''); setWelfareRefreshKey(k => k + 1); setWelfareView('home');
                          if (savedB64) {
                            (async () => {
                              try {
                                const byteChars = atob(savedB64);
                                ...
                                if (pdfRes.ok) {
                                  ...
                                  const a = document.createElement('a'); a.href = url;
                                  a.download = `דוח_גזבר_${savedMuniName}_${savedMonth}_${savedYear}.pdf`;
                                  a.click(); URL.revokeObjectURL(url);
                                }
            ...
            <div>
                    ...
                    {welfareIndexes.filter(i=>!welfareIndexSearch||i.key_value?.includes(welfareIndexSearch)||(i.connection_name||'').includes(welfareIndexSearch)||i.account_code?.includes(welfareIndexSearch)).filter((idx:any,i:number,arr:any[])=>arr.findIndex((x:any)=>x.key_value===idx.key_value)===i).map((idx:any)=>(
                      <tr key={idx.id} className="border-b border-gray-50 hover:bg-gray-50">
                        ...
                        {welfareEditId===idx.id ? (
                          ...
                          <td className="p-2"><input value={welfareEditVals.credit} onChange={e=>setWelfareEditVals(p=>({...p,credit:e.target.value}))} className="border rounded px-2 py-1 text-xs font-mono w-32" placeholder="חשבון חובה (184)"/></td>
                          <td className="p-2"><input value={welfareEditVals.debit} onChange={e=>setWelfareEditVals(p=>({...p,debit:e.target.value}))} className="border rounded px-2 py-1 text-xs font-mono w-32" placeholder="חשבון זכות (134)"/></td>
                          <td className="p-2 flex gap-1"><button onClick={async()=>{try{const dbRow=welfareIndexes.find(x=>x.key_value===idx.key_value&&x.description==='debit');const crRow=welfareIndexes.find(x=>x.key_value===idx.key_value&&x.description==='credit');if(welfareEditVals.debit&&dbRow)await apiFetch(`/indexes/${dbRow.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_code:welfareEditVals.debit,connection_name:welfareEditVals.name})});if(welfareEditVals.credit&&crRow)await apiFetch(`/indexes/${crRow.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_code:welfareEditVals.credit,connection_name:welfareEditVals.name})});setWelfareEditId(null);if(muni)apiFetch(`/indexes?municipality_id=${muni.id}&template_id=95b37d2d-c9ea-4ef1-9164-ab5ac642b0c7&limit=500`).then(d=>setWelfareIndexes(Array.isArray(d)?d:[]));} catch(e:any){setWelfareAddError(e.message);}}} className="text-xs bg-blue-600 text-white rounded px-2 py-1">שמור</button><button onClick={()=>setWelfareEditId(null)} className="text-xs border rounded px-2 py-1">ביטול</button></td></>
                        ) : (
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <div><label className="text-xs text-gray-500 block mb-1">חשבון חובה (184)</label><input value={welfareNewRow.credit} onChange={e=>setWelfareNewRow(p=>({...p,credit:e.target.value}))} className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-full font-mono" placeholder="184XXXXXXX"/></div>
                  <div><label className="text-xs text-gray-500 block mb-1">חשבון זכות (134)</label><input value={welfareNewRow.debit} onChange={e=>setWelfareNewRow(p=>({...p,debit:e.target.value}))} className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-full font-mono" placeholder="134XXXXXXX"/></div>
                </div>
